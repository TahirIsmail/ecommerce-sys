...
<h1 class="page-title">
    ...
    Hello world {{ __('voyager::generic.'.($edit ? 'edit' : 'add')).' '.$dataType->getTranslatedAttribute('display_name_singular') }}
</h1>
                ...
                <form role="form" class="form-edit-add" action="{{ $edit ? route('voyager.'.$dataType->slug.'.update', $dataTypeContent->getKey()) : route('voyager.'.$dataType->slug.'.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="panel-body">
                        ...
                        <ul style="list-style-type: none; padding-left: 0">
                            ...
                            <li><label for=""><input type="checkbox" name="categories[]" value="{{ $category->id }}" /> {{ $category->name }}</label></li>
                            @endforeach
                        </ul>
                    </div><!-- panel-body -->
...
<script>
    ...
    $('document').ready(function() {
        ...
        $('#confirm_delete').on('click', function() {
            $.post('voyager route not defined', params,
                function(response) {
                    if (response &&
                        response.data &&
